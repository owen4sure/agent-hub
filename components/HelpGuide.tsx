...
const STEPS: { icon: string; title: string; body: string }[] = [
  ...
  },
  {
    ...
    body: "草稿狀態下，按上方紫色的「🪄 幫我測到會跑」。它會實際跑一次（旁邊會跳出瀏覽器，你看得到它自己登入、點按鈕），哪一步失敗就自己修好再跑，直到整條通過。真的需要你補的（例如帳號密碼），它會明白告訴你要補什麼。",
  },
  {
    icon: "🔧",
    title: "4. 想改哪一步，點那一格講就好",
    body: "點畫布上任何一格，用白話說「改成抓另一封信」「顏色改藍色」，AI 只改那一步。某格變紅色代表失敗，點它按「讓 AI 修這一步」即可。節點可以拖動位置、雙擊改名字。",
  },
  {
    icon: "✅",
    title: "5. 測好了就「設為正式」＋設自動觸發",
    body: "確認結果沒問題，按「設為正式」把它固定下來。想讓它自動跑，按「⚡ 觸發」三選一：排程(每季/每月/每週的固定時間)、資料夾監聽(檔案丟進指定資料夾就跑)、Webhook(手機捷徑或其他程式打一個網址就跑)——都不用懂任何程式。跑完的產出檔在左邊「產出檔案」頁可直接拖到桌面。",
  },
  {
    icon: "🤖",
    title: "6. 自動跑失敗了怎麼辦？AI 已經先想好了",
    body: "正式流程自動觸發(排程/監聽/Webhook)執行失敗時，桌面會跳通知，AI 也會在背景先想好怎麼修。你只要打開首頁，看到「AI 已經想好怎麼修」那條，按「✅ 套用並重跑」確認一下就好，不用自己找問題、也不用自己去點修復。",
  },
];
          ...
          <div className="card w-full max-w-xl max-h-[88vh] flex flex-col" style={{ boxShadow: "var(--shadow-lg)" }}>
            ...
            <div className="flex-1 overflow-auto px-6 py-5 space-y-5">
              ...
              <div className="card px-4 py-3 text-[13px] leading-relaxed" style={{ background: "var(--surface-2)" }}>
                <b>第一次用建議這樣：</b>到左邊「草稿 & 範例」，把內建的「庫存 Excel 月結整理」按「複製」成自己的草稿，到「設定」填好信箱帳密，再按「🪄 幫我測到會跑」看它整套跑一遍，最有感覺。
              </div>
            </div>
