...
const STEPS: { icon: string; title: string; body: string }[] = [
  ...
  },
  {
    ...
    body: "AI 會先問清楚必要細節（哪個信箱、哪個日期…），確認後在左邊畫出一步一步的流程圖。從左到右就是執行順序：登入 → 找信 → 下載 → 處理 → 存檔。每一步就是一件要做的事，你看得到整件事怎麼跑。",
  },
  {
    icon: "🪄",
    title: "3. 按「測到會跑」讓它自己搞定",
    body: "草稿狀態下，按上方紫色的「🪄 測到會跑」。它會在背景實際跑一次（不會跳出視窗打擾你，進度看步驟面板），失敗時會把當下情況交給 AI 分析、改好後再測。需要你補帳號密碼或正確資料時，它會停下來直接說明原因和下一步，不會假裝成功。",
  },
  {
    icon: "🔧",
    title: "4. 想改哪一步，點那一步講就好",
    body: "點流程圖上任何一步，用白話說「改成抓另一封信」「顏色改藍色」。某步變紅色時，點它按「讓 AI 修」；AI 會同時看整條流程、實際輸入與出錯當下的情況，找到真正有問題的步驟（問題也可能在前面）再精準修改。每一步都可以拖動位置、雙擊改名字。",
  },
  {
    icon: "🎯",
  ...
  },
  {
    ...
    body: "流程需要帳密時(執行前發現沒填、或你在對話問「帳密在哪設定」)，對話會自動跳出「安全輸入卡」——直接在卡片欄位填，值只會存進你電腦的本機設定，不會出現在對話紀錄、也不會傳給 AI。左邊「設定」頁也能改，那一頁全部都是選填：流程用到哪個外部服務(Google、Telegram、LINE、Email…)才需要去串那一項，其餘不用管，什麼都不填也能開始建流程。",
  },
  {
    icon: "🔐",
          ...
          <div role="dialog" aria-modal="true" aria-labelledby="help-title" aria-describedby="help-description" className="card w-full max-w-xl max-h-[calc(100dvh-1.5rem)] sm:max-h-[88vh] flex flex-col" style={{ boxShadow: "var(--shadow-lg)" }} onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex-1 overflow-auto px-6 py-5 space-y-5">
              ...
              <div className="card px-4 py-3 text-[13px] leading-relaxed" style={{ background: "var(--surface-2)" }}>
                <b>第一次就從小事開始：</b>例如「我上傳 Excel 後，幫我算『金額』合計，只讓我看結果，不要改檔」。AI 做好後，先按「🪄 測到會跑」確認。
              </div>
              {!showDetails ? (
