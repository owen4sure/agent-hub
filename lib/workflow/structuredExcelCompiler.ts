...
export function parseDailyChannelMappings(intent: string): DailyChannelMapping[] | null {
  if (!intent.includes("客戶數A") || !intent.includes("客戶數B") || !intent.includes("通路統計")) return null;
  if (!/日期在\s*A\s*欄/.test(intent)) return null;

  const mappings: DailyChannelMapping[] = [];
  const seen = new Set<string>();
  const re = /(?:^|\n)\s*[-•]\s*([^\n：:]+?)\s*[：:]\s*客戶數A\s*([A-Z]+)\s*欄\s*\+\s*客戶數B\s*([A-Z]+)\s*欄/gm;
...
const twdSheet = workbook.getWorksheet("客戶數A");
const foreignSheet = workbook.getWorksheet("客戶數B");
const summary = workbook.getWorksheet("通路統計");
if (!twdSheet || !foreignSheet || !summary) throw new Error("日報缺少『客戶數A』『客戶數B』或『通路統計』分頁");
...
const summaryRowFor = (name) => {
  ...
  if (partial.length !== 1) {
    throw new Error("『通路統計』無法唯一對應「" + name + "」列；實際 B 欄有：" + labels.map((item) => item.label).filter(Boolean).join("、"));
  }
