...
test("口語執行參數：簡報內容裡的『本週成果』不是使用者指定的執行日期", () => {
  const result = extractChatRunParams("幫我做一張本週成果的簡報", rangeSchema, new Date("2026-07-14T02:00:00Z"));
  assert.equal(result.explicitRange, null);
