// builder 拆檔(2026-08)：建圖對話的 prompt 組裝層——從零建圖與既有圖修改的兩份系統提示、
// 各 *Section 片段、MAIL_RECIPE/IMMEDIATE_BUILD_CONTRACT 等兩份提示共用的文案，以及把圖與
// 對話歷史壓成提示的 compactGraphJson/trimHistoryForBuilder。只組字串，不呼叫模型。
...
import { getBuilderPrefs, getSharedSecrets } from "../settingsStore";
...
import type { ChatMessage, MessagePart, RuntimeContext } from "./builderTypes";
 ...
 */
export const IMMEDIATE_BUILD_CONTRACT =
  ...
  "④同一種欄位名可能出現多次時(例如流程裡有兩個以上的下載步驟,每個都輸出 attachmentPath)，下游要引用「特定那一步」的值必須寫 {{步驟代號.欄位}}(例如 {{n3.attachmentPath}})——{{欄位}} 攤平引用拿到的是「離自己最近的上游」的值,較早那步的會被蓋掉。單一來源的欄位照舊用 {{欄位}} 即可。\n" +
...
export function compactGraphJson(graph: { nodes: WorkflowNode[]; edges: WorkflowEdge[] }, exactKeep: string[] = [], fuzzyKeep: string[] = [], ceilingChars = CODE_CEILING_CHARS): CompactGraph {
...
export function secretsStatusSection(status?: { key: string; label: string; filled: boolean }[]): string {
...
- **變數引用預設用「扁平欄位名」:{{欄位名}}**。{{節點代號.欄位}} 只在兩種情況合法:①代號是**自己上游(祖先)**的節點——用於同名欄位有多個來源時精準取「那一步」的值;②repeat-steps 裡的 {{item.*}}。指向「不是自己上游」的節點(如平行分支/下游)執行期解析不到、條件會永遠走錯分支,建圖檢查會直接擋下。{{period.*}} 只能放在期間參數的 derived default。
