...
test("Mail2000 重新登入：帳號已預填且 disabled 時不會再 fill 到逾時", async () => {
  ...
  try {
    ...
    await page.setContent('<input name="USERID_show" value="user@example.com" disabled>');
    const logs: string[] = [];
    const result = await fillAccountField(page, 'input[name="USERID_show"]', "user@example.com", (line) => logs.push(line));
    assert.equal(result, "prefilled");
    assert.equal(await page.locator('input[name="USERID_show"]').inputValue(), "user@example.com");
