...
import { PARSE_RULES } from "./codegen";
...
export function applyNodeConfigEdits(
  ...
  for (const e of rawEdits) {
    ...
    edits.push({ nodeId: node.id, nodeType: node.type, nodeLabel: node.label, before: { ...node.config }, after: newConfig });
    workingNodes = workingNodes.map((n) => (n.id === node!.id ? { ...n, config: newConfig } : n));
  }
  if (edits.length > 0 && opts.apply !== false) {
    saveWorkflow({ ...fresh, nodes: workingNodes });
  }
...
修 repeat-steps 內嵌步驟時的元素格式：{"nodeId":"repeat-steps節點id","stepIndex":第幾步(0起),"config":{ 那一步改好後的 config }}`;
  ...
  if (isClaudeCodeModel(model)) {
    raw = await callAIWithRetry(claudeCodeFallback, { label: "整圖修復(Claude Code)", signal: opts.signal });
  } else {
