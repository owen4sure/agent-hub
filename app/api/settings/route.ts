...
import { getGlobalSettings, setGlobalSettings, getMaxConcurrent, setMaxConcurrent } from "@/lib/settingsStore";
...
export async function GET() {
  ...
  return NextResponse.json({ baseUrl, hasApiKey: Boolean(apiKey), maxConcurrent: getMaxConcurrent(defaultMaxConcurrent()) });
}

export async function POST(req: Request) {
  const body = (await req.json().catch(() => null)) as { baseUrl?: string; apiKey?: string; maxConcurrent?: number } | null;
