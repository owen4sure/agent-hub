...
export async function POST(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (autorunActive.has(id) || hasActiveRepairSession(id)) return NextResponse.json({ error: "自動測試／修復進行中，等它完成再啟用 Webhook" }, { status: 409 });
  if (wf.status === "official") {
    const readiness = getAutomationReadiness(wf, "webhook-enable");
...
export async function DELETE(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (autorunActive.has(id) || hasActiveRepairSession(id)) return NextResponse.json({ error: "自動測試／修復進行中，等它完成再停用 Webhook" }, { status: 409 });
  disableWebhook(id);
