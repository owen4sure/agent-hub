...
- **Test just a segment (n8n-style)**: click a node and hit "▶ test from this step" or "▶ test only this step"; or drag-select several nodes on the canvas and hit "▶ test only these". Unselected steps reuse their latest results or are skipped honestly — when you add a new tail section you don't have to re-run login/fetch/fill every time just to verify it. Partial tests run headless by default (no browser window stealing your screen); tick "watch" to see the browser work.

## Credentials, and logging into Google-like sites

- **Credentials go into a secure input card, never into the chat**: when a flow needs credentials (a run is blocked on missing ones, or you ask the chat where to set them), the chat shows a secure input card — values are stored only in your local settings (SQLite), never appear in the chat history, and are never sent to any AI model. Secret fields referenced by custom-code steps are discovered automatically and appear on the Settings page.
- **Google/Microsoft need a one-time manual login**: these platforms detect and block automated logins — even with a correct password you'll see "Couldn't sign you in / This browser or app may not be secure". That's not your flow being broken, and retrying won't help. The fix: workflow page → "⋯ → 🔐 manual login once", sign in by hand in the window that opens, then close it; the logged-in state is saved into the flow's browser session and every later run starts already signed in. The AI builder is also explicitly taught to never design steps that type Google passwords.
