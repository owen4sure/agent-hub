...
- **新流程的預設模型由 `defaultModelRef()` 推導，不是常數**：有填內建 Base URL/金鑰就沿用 `DEFAULT_MODEL`（既有使用者行為完全不變），沒填就預設 Claude Code。這一條**取代**了先前「Claude Code 不是主力、別設成預設模型」的守則——那條是在「大家都有這個免費 gateway」的前提下寫的，而這個專案開源之後，一個剛 clone 下來、只裝了 Claude Code 的人若拿到 `minimax-m3` 當預設，會在他的環境裡叫不動，打第一句話就失敗且看不出原因。「其他人只要預設 Claude Code，其他模型自己去設定裡加」是產品決定，不要再退回寫死的預設值。
...
## 「這一步用哪顆模型」只能問 `lib/modelPolicy.ts`（2026-08 定下）

執行期任何要挑模型的地方，一律走 `planNodeModel(ctx, need, 節點指定的)` + `runWithModelChain(...)`（兩者都在 `lib/workflow/nodeHelpers.ts`）。**節點檔案裡不准再出現任何寫死的模型名稱或備援順序**——包括 `?? VISION_MODELS[0]` 這種保底寫法。

- **順序是使用者的，不是平台的**：主力/救援由設定頁的排序決定（`ModelPreference`），流程層可以指定「跑起來用哪顆」（`wf_model.run_model`），節點層可以再指定「這一步用哪顆」。三層都留空才輪到 `autoOrder` 預填，而那個預填只依「實測過的能力 → 使用者標記的地端 → 內建可靠度」這些可觀察事實排，不依模型名字。
- **`lib/models.ts` 的 `VISION_MODELS`/`KNOWN_WORKING_MODELS`/`DEFAULT_MODEL` 只描述內建那一個 gateway**，不是平台的事實來源。判斷某顆模型會不會看圖用 `modelSupportsVision()`（實測優先），不要用 `supportsVision()`（靜態清單）——後者認不得使用者自己接的模型，會讓「已經指定地端模型」的流程被靜默改送到別的端點。
- **挑不到就回空的鏈＋白話原因，絕不退回一個寫死的名字**。只裝了 Claude Code 的人做含驗證碼的流程，正確行為是講清楚「你手上沒有能解驗證碼的模型，可以接一顆或改用手動登入」，不是去打一個他沒有的服務然後回「金鑰不對」。
- **每一次呼叫都要寫進執行紀錄「用了誰、地端還是雲端、為什麼換」**（`runWithModelChain` 已經統一做掉）。這不是紀錄品質問題——使用者要拿執行紀錄去做資料落地確認，靜默換模型等於前面所有設定都白費。
- **嚴格模式（`strict`）下不准有任何 fallback**：做不到就 `PermanentError` 停下來。反過來說，**沒勾的時候不要拿掉自動換備援**——對多數使用者那是流程不斷掉的關鍵。
- **來源被刪掉/改名時絕不「順手挑一顆同名的」**：同名模型可能同時存在共用 gateway 上，靜默頂替 = 業務資料送去另一個端點、還換了一顆模型做判斷，而執行紀錄完全看不出來。`planModelChain` 的 `find()` 已經釘住「純代號只在全平台唯一時才算數」。
