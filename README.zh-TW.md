...
「▦ 範本庫」有 10+ 條精選範本(簽核流程/失敗告警/AI 分流/單據辨識/逐項處理/晨報…),每條都經過確定性 lint 把關。挑一個接近的按「使用這個範本」變成自己的草稿,再用白話跟 AI 說哪裡不一樣。流程多了之後:首頁可搜尋、可分群組(工作/私人…)分區顯示;「☰ 執行紀錄」一頁看完所有流程的歷史執行。

![範本庫:分類瀏覽、步驟預覽、一鍵複製成草稿](docs/screenshots/templates.png)
