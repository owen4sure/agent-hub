...
「▦ 範本庫」有 18 條通用範本,型態取材自 n8n 社群工作流庫的最大宗用途(監控告警/資料收集/AI 內容/報表彙整/審核把關…),每條都經過確定性 lint 把關。挑一個接近的按「使用這個範本」變成自己的草稿,再用白話跟 AI 說哪裡不一樣。流程多了之後:首頁可搜尋、可分群組(工作/私人…)分區顯示;「☰ 執行紀錄」一頁看完所有流程的歷史執行。
