...
「▦ 範本庫」有 58 條通用範本,直接改編自 n8n 社群工作流庫(Zie619/n8n-workflows,2,000+ 條)的實際工作流並標注原始出處——監控告警/整合串接/行銷業務/報表彙整/檔案自動化/開發者工具…九大分類,每條都經過確定性 lint 把關(CI 閘門)。另外,AI 建流程時會自動從**全庫 2,061 條的本地索引**檢索最相近的同型流程當參考藍圖——你問到任何常見自動化,AI 手上都有真實世界的結構可對照。挑一個接近的按「使用這個範本」變成自己的草稿,再用白話跟 AI 說哪裡不一樣。流程多了之後:首頁可搜尋、可分群組(工作/私人…)分區顯示;「☰ 執行紀錄」一頁看完所有流程的歷史執行。
