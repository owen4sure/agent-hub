...
單人本機的視覺化 workflow 自動化平台，把**正式環境該有的可靠性帶到個人與小團隊的自動化**——**像 n8n 一樣**在畫布上把一件事拆成一個個節點串成流程，但**整個建構與修改都用白話跟 AI 完成，你永遠不寫、不看程式碼**。它投資的地方：確定性驗證、重試、自我修復迴圈、查得到的執行證據、一鍵還原。它刻意停在哪裡：不是多人雲端版，沒有帳號系統／RBAC／SSO，而且**設計上就會在你的電腦上執行 AI 寫的程式碼**。要在共用或公司控管的機器上跑，請先讀 [SECURITY.md](./SECURITY.md)。
...
- **每人本機自架**：clone 下來自己跑，帳密/API key 只存在你自己電腦的 `data/`(gitignore)，永遠不會被送到模型 API。
- **「本機」到什麼程度(該講清楚的部分)**：流程執行是在本機。但只要 AI 有參與——建流程、產自訂步驟的程式碼、`llm-decide` 判斷、按「讓 AI 修」——執行現場的資料(節點收到的 input、頁面 HTML、失敗截圖)就會被送到 `AGENT_HUB_BASE_URL` 指向的模型端點。不希望資料離開這台電腦，就把它指向地端模型。完整的資料流對照表在 [SECURITY.md](./SECURITY.md#42-執行現場資料會送到外部-ai-api)。
