...
示範把一件事拆成 5 個節點：開始 → 登入 webmail(圖形驗證碼 AI 自動辨識) → 用日期+報表名稱精準找信 → 下載附件 → Excel 篩選區間+highlight+存檔。可「複製」來改成自己要的。日期參數支援相對變數(`{{yesterday}}`、`{{last-quarter-start}}`)，排程觸發時自動算成當下日期；每次執行前可選結算期間，日期自動算好給你確認。

![執行前選期間：篩選起訖、報表日期、輸出檔名全部自動算好](docs/screenshots/run-form.png)

## 節點庫(可持續擴充)

觸發、瀏覽器(登入/找信/下載附件)、資料(Excel/組字串/PDF讀取/zip解壓縮)、整合(打API)、邏輯(條件/變數)、AI(判斷/產生)、自訂程式碼。每種節點是一個 `lib/workflow/nodes/*.ts` 檔，加一個檔 AI 立刻就會用。上傳給 AI 的檔案(PDF、Word `.docx/.doc`、Excel `.xlsx/.xls`、PowerPoint `.pptx`、RTF、純文字家族)會在伺服器端直接抽成文字讓 AI 讀懂(`lib/textExtract.ts`)；圖片/截圖走視覺模型。

## 排程

在 workflow 設排程(每天/每月/每季 1,4,7,10 月/每週/cron)，時間到自動觸發整張圖(headless)。需要引擎常駐：
...
```bash
npm run test    # 跑核心邏輯的單元測試(相對日期解析、節點圖檢查、JSON 抽取、cron 等純函式)
npm run lint    # ESLint
```
